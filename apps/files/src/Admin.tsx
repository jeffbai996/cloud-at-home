...
export function AdminPanel({ initialTab = "users", currentUserId, usage }: { initialTab?: "users" | "shares" | "settings"; currentUserId: string; usage: StorageUsage | null }) {
  ...
  return (
    <div className="admin-panel">
      ...
      {tab === "shares" && <div className="admin-section"><div className="admin-section-heading"><div><strong>Shared links</strong><span>Links created from Cloud at Home Drive</span></div></div><div className="admin-list">{shares.length ? shares.map((share) => <div className="admin-row" key={share.hash}><span className="avatar share-avatar"><ExternalLink size={16} /></span><div><strong>{share.path}</strong><span>{share.expire ? `Expires in ${share.expire} ${share.unit ?? "hours"}` : "No expiration"}</span></div><code>{share.hash.slice(0, 8)}…</code></div>) : <div className="admin-empty">No active shared links.</div>}</div></div>}
